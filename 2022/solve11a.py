...
from typing import TextIO, List, Optional, Callable, Tuple
...
def parse_operation(f: TextIO) -> Callable:
    ...
    if op1 == "old" and op2 == "old":
        if operator == "*":
            return lambda old: (old * old) % large_modulo
        elif operator == "+":
            return lambda old: (2 * old) % large_modulo
    else:
        if op2 == "old":
            op1, op2 = op2, op1
        ...
        if op1 != "old":
            op1 = int(op1)
            if operator == "*":
                return lambda old, op1=op1, op2=op2: (op1 * op2) % large_modulo
            elif operator == "+":
                return lambda old, op1=op1, op2=op2: (op1 + op2) % large_modulo
        else:
            if operator == "*":
                return lambda old, op2=op2: (old * op2) % large_modulo
            elif operator == "+":
                return lambda old, op2=op2: (old + op2) % large_modulo


def parse_test(f: TextIO) -> Tuple[Callable, int]:
    line = read_line(f)
    ...
    test = lambda count, modulo=modulo: (count % modulo) == 0
    return test, modulo
...
def parse_monkey(f: TextIO, idx_hint: int) -> Monkey:
    ...
    test, test_modulo = parse_test(f)
    target_true = parse_if(f, "true")
    target_false = parse_if(f, "false")

    return Monkey(items, operation, test, test_modulo, [target_true, target_false])
...
# do all operations modulo the product of the factors encountered in the tests
# XXX using large_modulo as global variable: grr!
large_modulo = math.prod([monkey.test_modulo for monkey in monkeys])
print(f"large modulo is {large_modulo}")
