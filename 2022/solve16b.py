...
best = {(0, start, start): 0}
full_state = 2 ** len(flows) - 1
...
for k in tqdm(range(n_steps)):
    ...
    for (valve_state, node_me, node_ele), released in best.items():
        further_release = sum(
            flow for i, flow in enumerate(flows.values()) if valve_state & (2**i) != 0
        )
        value = released + further_release

        if valve_state == full_state:
            # just stay here and do nothing
            key = (valve_state, node_me, node_ele)
            new_best[key] = max(new_best.get(key, 0), value)
            continue

        # could go through a tunnel...
        actions_me = list(graph[node_me])
        actions_ele = list(graph[node_ele])
        ...
        for action_me in actions_me:
            for action_ele in actions_ele:
                if action_me[0] != "*" and action_ele[0] != "*":
                    ...
                    if new_valve_state != valve_state:
