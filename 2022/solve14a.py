...
# gap between lowest point and floor
gap = 2

n_rows = rows[1] + gap - rows[0] + 1

# adjust to be able to include a full triangle of sand
cols[0] = min(cols[0], origin[1] - n_rows + 1)
cols[1] = max(cols[1], origin[1] + n_rows - 1)

# add a segment at the bottom
segments.append(((rows[1] + gap, cols[0]), (rows[1] + gap, cols[1])))
